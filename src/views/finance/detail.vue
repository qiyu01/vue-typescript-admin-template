...
import DetailFilter from './components/DetailFilter.vue'
...
@Component({
  name: 'FinanDetail',
  components: {
    Cards, MsgList, DetailFilter, DetailList
  }
})
export default class extends Vue {
  private tab='1'
  private detailtabs='first'
  ...
  private async getAllFinance(params?:any) {
  ...
  private async getBillmsg(params?:any) {
    this.billMsg.loading = true
    var obj:BillMsgQuery = {
    ...
    if (this.billMsg.timeCopy.length > 0) {
  ...
  private billMsgPageChange(page:number) {
    var params = {
      pageNo: page
    }
  ...
  private billMsgSizeChange(size:number) {
    var params = {
      pageSize: size
    }
  ...
  private async getBillDetail(params:any, type:number) {
